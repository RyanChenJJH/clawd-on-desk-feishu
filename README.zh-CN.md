...
<p align="center">
  <sub>🍴 这是 <a href="https://github.com/rullerzhou-afk/clawd-on-desk">Clawd 桌宠</a>（作者 鹿鹿 <a href="https://github.com/rullerzhou-afk">@rullerzhou-afk</a> 及众多贡献者）的一个社区分支 —— <b><a href="https://github.com/RyanChenJJH/clawd-on-desk-feishu">clawd-on-desk-feishu</a></b>。原项目的全部功劳归原作者所有；本分支仅在其之上<b>新增</b>了可选、默认关闭的功能（健康提醒 + 飞书远程审批，见下文），并持续同步上游更新（当前已同步至 <b>v0.10.0</b>）。</sub>
</p>
...
## 本分支扩展功能（Expand_function）

> 本节功能由本分支（[clawd-on-desk-feishu](https://github.com/RyanChenJJH/clawd-on-desk-feishu)）在原项目 [Clawd 桌宠](https://github.com/rullerzhou-afk/clawd-on-desk)（作者 鹿鹿 [@rullerzhou-afk](https://github.com/rullerzhou-afk)）之上**新增**。这些功能均为**独立、可选、默认关闭**——全部关闭时，Clawd 的行为与上游完全一致。本分支会定期合并原作者的更新，最近一次上游合并记录见 [`docs/update_combine/`](docs/update_combine/)。

- **🧋 健康提醒（Health Reminder）** —— 定时发出温和的健康提醒（喝水、起身、拉伸、护眼、午餐、下班……），每条提醒都有独立、常驻、可交互的气泡卡片与配套桌宠动画。支持预设库及 JSON 导入/导出、勿扰/安静时段感知、基于空闲状态的"智能"调度、每条提醒独立音效，以及专属的 **设置 → 健康提醒** 标签页。默认关闭。
- **✈️ 飞书远程审批（Feishu / Lark Remote Approval）** —— 将权限审批流程镜像到飞书（Lark）。当受支持的 agent 请求工具权限时，会向授权审批人发送交互式卡片，审批人可直接在飞书上 **同意 / 拒绝**，甚至回答 `AskUserQuestion` 提问，结果会同步回桌面。它作为与现有 Telegram 路径并列的 provider 实现，位于 `src/remote-approval/`；凭据仅保存在本机；默认关闭。

> 这些扩展遵循原项目的结构与许可证，并尽量保持易于与上游合并。Clawd 形象及全部随附美术资源的版权归各自作者所有——见 [许可证](#license)。
